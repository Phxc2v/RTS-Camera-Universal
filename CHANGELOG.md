...
In‑game build **1.1.2.4**. The universal build matured into **one byte‑identical folder that runs on Bannerlord 1.2.x, 1.3.15 and 1.4.5**, auto‑selecting the right code, GUI and assets at runtime — plus a wave of crash fixes (first‑launch, order‑UI brushes, vanilla "empty column", The Old Realms spell‑cast, formation placement).
...
### Added
- **True single universal module — one byte‑identical folder for Bannerlord 1.2 / 1.3 / 1.4.** A single download now works on all three versions as is: a version‑neutral core loader detects the game version at launch and loads the matching payload, GUI / prefabs and asset package. The version‑specific order‑UI prefabs ship side by side under `GUI/Prefabs/Versioned/`; the mesh package (`pack0.tpac`) ships in the 1.2‑safe variant and the loader swaps in the running version's package before the engine reads it. One launcher checkbox; no per‑version downloads, no differing installs.
...
### Fixed
- **First‑launch crash on Bannerlord 1.3/1.4 eliminated.** On a freshly installed module the engine loaded the 1.2‑safe mesh package before any mod code could swap in the matching version's package, so the very first battle load could crash. The first launch now boots with the correct package in place; later launches self‑heal as before.
